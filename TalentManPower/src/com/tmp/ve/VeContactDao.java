...
public class VeContactDao {
	...
	public static void insertDetails(VeContact veContact, Connection con) {
		...
		try{
			PreparedStatement ps=con.prepareStatement("insert into ve_contactus (roles,work_domain,experience,tenure,shift_time"
					+ "full_part_time,start_time,technical_proficiency,outsourcing,outsourcing_details,business_entity,business_domain"
					+ "time_of_operation,annual_turnover) values	('"+veContact.getRoles()+"','"+veContact.getWorkDomain()+"','"+veContact.getTenure()+"','"+veContact.getShiftTime()+"','"+
